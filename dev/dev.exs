...
repo_config =
  ...
  |> Keyword.put_new(
    :username,
    System.get_env("POSTGRES_USER") || System.get_env("USER") || "postgres"
  )
...
children =
  [{Phoenix.PubSub, name: ObanUI.DevApp.PubSub}, ObanUI.DevApp.Repo] ++
    oban_children ++
    [
      {ObanUI, oban_names: oban_names, pubsub: ObanUI.DevApp.PubSub, repo: ObanUI.DevApp.Repo},
      ObanUI.DevApp.Endpoint
    ]

{:ok, _sup} =
  Supervisor.start_link(children, strategy: :one_for_one, name: ObanUI.DevApp.Supervisor)
